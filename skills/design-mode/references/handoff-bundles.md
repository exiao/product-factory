...
Translate visual values, behavior, and flow into the destination codebase's framework. Prototype Babel/CDN scaffolding, browser globals, device frames, tweak controls, and host messaging are not automatic production requirements. Reuse current project tokens and resolve conflicts explicitly; historical Bloom bundle colors do not override the active bloom-design-system or user-selected design.
