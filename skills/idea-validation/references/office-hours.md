# Optional office-hours lenses

Use the section that changes the current decision. These are adapted methods from the exported Sahil Office Hours and YC Office Hours skills, not simulated access to either advisor and not endorsements. The Sahil source attributes its approach to Sahil Lavingia's *The Minimalist Entrepreneur* and `slavingia/skills`. The YC-style source combines discovery questions with ambitious product thinking; treat it as a framework, not official YC advice.

## Bootstrap and first customers: Sahil-inspired

Start from people whose problem you understand and can realistically reach. Existing community membership can help, but is not a prerequisite. Use specific buyer segments and observed workarounds; do not invent named prospects or equate friends' enthusiasm with demand.

Ask whether a small manual service can test the value before investing in automation. If useful, sketch:

1. The trigger and what the customer supplies.
2. The actual steps, tools, and judgment required.
3. The result delivered and its quality standard.
4. Time and cost per customer, including support and founder labor.
5. The proposed price or commitment and what would justify repeating the process.
6. Which proven step would be worth automating next.

Manual delivery is a test option, not a universal requirement. Some products need software to demonstrate their core value; some manual pilots test a different service entirely. Identify what the pilot can and cannot establish. Do not require a weekend build, ten interviews, or a fixed paying-customer count as a universal gate.

A paid pilot can test willingness to pay more directly than praise. Free trials, freemium, public goods, and other models can still be appropriate. Judge price together with fulfillment costs and buyer value; do not mandate starting cheap or always charging. Track evidence of repeat use and sustainable delivery separately from a first sale.

For finding initial customers, choose qualified, reachable people and a relevant channel. Warm introductions may help, but friends and family are neither required buyers nor a representative market. Draft outreach only when helpful to the request; contacting people, collecting payment, or delivering a service requires that authorization. No bulk-contact quotas or automatic sending.

For a bootstrap decision, examine customer benefit, simplest viable delivery, reversibility, time versus money, economics, and fit with the founder's goals. Profitability can preserve control, but does not make runway literally unlimited. Ads, hiring, fundraising, and launch timing are contextual choices, not prohibited until arbitrary milestones.

Useful output: the smallest credible offer, delivery sketch, biggest risk, and one test to run next. A fixed launch threshold or a broad marketing calendar is unnecessary for a narrow validation question.

## Discovery and scope: YC-style

Use the questions the existing evidence has not answered:

- Who specifically needs this, and in what situation?
- What do they do today, and what is unsatisfactory about it?
- Who has the strongest need or willingness to act, and what evidence supports that?
- What is the smallest version that delivers the actual value?
- What observation challenges the usual assumption about this problem?
- Could this grow into a product or is it better as a feature, service, or internal tool?

Answer from supplied research or code before asking the user. Do not assume the literal request is wrong or force a reframe. Turn material assumptions into falsifiable premises and state the evidence that would overturn them. Preserve accepted decisions instead of repeatedly seeking approval.

Match scope behavior to the request:

| Intent | Behavior |
| --- | --- |
| Hold scope | Apply rigor to the agreed work without introducing expansion. |
| Reduce | Find the smallest coherent outcome and defer the rest. |
| Explore selectively | Compare a few worthwhile extensions to the existing baseline. |
| Expand | Explore an ambitious experience, then find a feasible first step. |

Infer an already clear intent instead of making the user select a mode. For an implementation request or locked plan, hold scope. Ambitious or “10-star” thinking applies when exploration is requested; it is not a reason to replace an ordinary ticket with a larger product. A proposed expansion remains optional until selected or its choice has been delegated.

When alternatives matter, compare a few approaches by what ships, what is learned, dependencies, effort uncertainty, and what is deferred. Estimate from actual work and constraints; do not invent human-versus-AI speedups. Recommend one and explain the tradeoff.

Record an agreed plan only when the task needs a durable handoff, using the project's existing location and conventions. Include problem, premises, selected scope, first increment, evidence plan, and open questions. Avoid mandatory personal reflections, duplicate vision documents, or a new approval ceremony.
