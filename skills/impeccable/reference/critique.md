...
6. **Close the run.** If the Authorized follow-through branch below applies, present findings and continue into the authorized fixes. Otherwise, go to Ask the User below and emit the questions, or the `Questions skipped: <reason>` line when the count allows it. The run is not complete until you do. Persistence is bookkeeping and cleanup is not an ending; stopping here leaves the user with a report and no way forward, and leaves `$impeccable polish` with no priorities to inherit.

This is fire-and-forget. Do not show the user the helper's JSON output; only the human-readable trend line and the written path. Failures here should not block the rest of the flow; print the error and move on.

### Authorized follow-through

When the original request already asks for critique followed by implementation and defines the scope (for example, “fix the highest-impact problems within the existing design”), present the findings and proceed with the relevant fix playbook. This branch overrides the question count and final-question gate below; do not ask the user to repeat authorization or choose priorities they already delegated. Ask only about material ambiguity that prevents the scoped work. Preserve the report and snapshot steps, keep fixes within the authorized scope, and verify through the selected playbook. A critique-only request remains a review and does not authorize edits.
