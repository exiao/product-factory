...
For still-image generation in this environment, follow [image-generation-guide](../../../image-generation-guide/SKILL.md) and use the Codex built-in tool. The API fallback and `impeccable generate-image` alternatives below are not the default and require an explicit user request for that provider. Local cropping, metadata, and validation utilities remain applicable. Save the generated result at the declared local comp or plate path.
