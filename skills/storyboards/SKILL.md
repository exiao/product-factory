...
Use the notebook-sketch (optional, when installed) visual language in [references/notebook-style.md](references/notebook-style.md) unless the user supplies another style. Default to a three-column, two-row composition for six panels. Keep captions short; specify exact labels and any required screen text. Do not invent product features, tickers, numbers, or successful outcomes as factual evidence.
