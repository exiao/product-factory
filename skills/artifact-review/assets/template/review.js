...
function payload(){return {...state,project:config.title,artifacts:config.artifacts.map(({id,version,type,selected,settled})=>({id,version,type,selected,settled}))}}
async function post(endpoint,body=payload()){const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok)throw Error(data.error||'Could not save. Your notes are preserved.');return data}
...
function render(item,nav){const a=config.artifacts.find(x=>x.id===item.id),k=key(a);const draft=state.drafts[k]||{notes:'',directions:a.selected||[]};const choices=new Set(draft.directions||[]);const actionDrafts=draft.actions||{};const root=node('article');root.append(node('h1',a.question||a.title));
  ...
  async function respond(intent){saveDraft();if(intent==='revision'&&!notes.value.trim()){message.textContent='Describe the change you want.';notes.focus();return}if(intent==='direction'&&!choices.size&&!notes.value.trim()){message.textContent='Select an option or describe your direction.';notes.focus();return}const response={artifact:a.id,version:a.version,intent,notes:notes.value,directions:[...choices],at:new Date().toISOString()};keep.disabled=revise.disabled=true;message.textContent='Saving…';try{await post('/api/review',{...payload(),responses:{...state.responses,[k]:response}});state.responses[k]=response;local();nav.advance()}catch(e){message.textContent=e.message;keep.disabled=revise.disabled=false}}
