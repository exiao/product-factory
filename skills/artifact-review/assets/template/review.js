...
let cleanup;
function render(item,nav){cleanup?.();const a=config.artifacts.find(x=>x.id===item.id),k=key(a);const draft=state.drafts[k]||{notes:'',directions:a.selected||[]};const choices=new Set(draft.directions||[]);const actionDrafts=draft.actions||{};const root=node('article');root.append(node('h1',a.question||a.title));
